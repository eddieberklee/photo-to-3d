...
export type GenerationState = 'idle' | 'uploading' | 'processing' | 'generating' | 'complete' | 'error';
...
export default function GenerationStatus({ status, progress, error, onRetry }: GenerationStatusProps) {
  ...
  const isIndeterminate = ['uploading', 'processing'].includes(status) && progress === undefined;
      ...
      <div className="bg-zinc-900/80 backdrop-blur-sm border border-zinc-800 rounded-2xl p-6">
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          <div className="flex-1">
            ...
            {status === 'generating' && (
              <p className="text-sm text-zinc-500 mt-0.5">This may take a minute...</p>
            )}
          </div>
          {progress !== undefined && (
        ...
        <div className="h-2 bg-zinc-800 rounded-full overflow-hidden">
          {isIndeterminate ? (
            <div
              className="h-full bg-gradient-to-r from-blue-500 via-purple-500 to-blue-500 animate-shimmer"
        ...
        {status === 'error' && (
          <div className="mt-4">
            {error && <p className="text-sm text-red-400/80 mb-3">{error}</p>}
            {onRetry && (
              <button
                onClick={onRetry}
            ...
            {['Depth', 'Mesh', 'Texture', 'Refine'].map((step, i) => {
              ...
              return (
                <div key={step} className="flex flex-col items-center gap-1">
                  ...
                  />
                  <span
                    className={`text-xs ${isComplete || isCurrent ? 'text-zinc-300' : 'text-zinc-600'}`}
                  >
                    {step}
                  </span>
                </div>
